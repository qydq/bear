...
/**
 * 请关注个人知乎bgwan， 在【an情景】专栏【ali】框架模块会有详细的使用案例（20190922-正在持续更新中...）
 * <p>
 * 中文描述：3d滚轮控件
 ...
 */
public class INAWheelView extends View {
    ...
    public INAWheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        textColorOut = getResources().getColor(R.color.an_color_wheel_out);
        textColorCenter = getResources().getColor(R.color.an_color_wheel_center);
        dividerColor = getResources().getColor(R.color.an_color_line);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        try {
            ...
            LaLog.d("出错了！adapter.getItemsCount() == 0，联动数据不匹配");
        }
        if (!isLoop) {//不循环的情况
        ...
        if (label != null) {
            ...
            if (mGravity == Gravity.LEFT || mGravity == Gravity.START) {
                canvas.drawText(label, (getTextWidth(paintCenterText, label) + 11), centerY, paintCenterText);
            } else {
                //靠右并留出空隙
    ...
    private void measuredCenterContentStart(String content) {
        ...
        switch (mGravity) {
            ...
            case Gravity.LEFT:
                ...
                break;
            case Gravity.RIGHT:
                drawCenterContentStart = measuredWidth - rect.width() - (getTextWidth(paintCenterText, label) + 20);
                break;
        }
    }
    ...
    private void measuredOutContentStart(String content) {
        ...
        switch (mGravity) {
            ...
            case Gravity.LEFT:
                ...
                break;
            case Gravity.RIGHT:
                drawOutContentStart = measuredWidth - rect.width() - (getTextWidth(paintCenterText, label) + 20);
                break;
        }
    }

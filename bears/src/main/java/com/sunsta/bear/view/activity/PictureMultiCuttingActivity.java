...
public class PictureMultiCuttingActivity extends UCropActivity {
    ...
    private void addPhotoRecyclerView() {
        ...
        RelativeLayout.LayoutParams lp =
                new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.MATCH_PARENT, ScreenUtils.dip2px(80));
    ...
    protected void resetCutData() {
        ...
        String suffix = MimeType.getLastImgType(MimeType.isContent(path)
                ? FileUtils.INSTANCE.getPath(this, Uri.parse(path)) : path);
        Uri uri;
        ...
        extras.putParcelable(UCrop.EXTRA_OUTPUT_URI,
                Uri.fromFile(new File(file,
                        TextUtils.isEmpty(renameCropFilename) ? FileUtils.INSTANCE.getCreateFileName("IMG_") + suffix : isCamera ? renameCropFilename : FileUtils.INSTANCE.rename(renameCropFilename))));
        intent.putExtras(extras);
        setupViews(intent);
        refreshPhotoRecyclerData();
        setImageData(intent);
        setInitialState();
        int scrollWidth = cutIndex * ScreenUtils.dip2px(60);
        if (scrollWidth > mScreenWidth * 0.8) {
            mRecyclerView.scrollBy(ScreenUtils.dip2px(60), 0);

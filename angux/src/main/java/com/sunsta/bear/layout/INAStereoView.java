 ...
 * <p>
 * 中文描述：【an情景】系列【alidd框架】提供的StereoView该效果就是一个水立方滑动可以旋转的动态图
 ...
 * @version 1.0 |   2016/11/25           |   滑动的旋转图水立方。
...
public class INAStereoView extends ViewGroup {
    ...
    public INAStereoView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public INAStereoView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void toNextAction(float yVelocity) {
        ...
        LaLog.m("多后一页startY " + startY + " yVelocity " + yVelocity + " delta " + delta + "  getScrollY() " + getScrollY() + " addCount " + addCount);
        duration = (Math.abs(delta)) * 3;
        mScroller.startScroll(0, startY, 0, delta, duration);
        addCount--;
    }
     ...
     * @return
     */
    public INAStereoView setStartScreen(int startScreen) {
     ...
     * 设置移动阻力
     * @param resistance (0,...)
     * @return 当前INAStereoView
     */
    public INAStereoView setResistance(float resistance) {
     ...
     * @return
     */
    public INAStereoView setInterpolator(Interpolator mInterpolator) {
     ...
     * @return
     */
    public INAStereoView setAngle(float mAngle) {
     ...
     * @return
     */
    public INAStereoView setCan3D(boolean can3D) {
        isCan3D = can3D;
        return this;
    }
     ...
     * @return
     */
    public INAStereoView setItem(int itemId) {

        LaLog.m("之前curScreen " + mCurScreen);
        if (!mScroller.isFinished()) {
            mScroller.abortAnimation();
            LaLog.m("强制完成");
        }
        ...
        LaLog.m("之后curScreen " + mCurScreen + " getScrollY " + getScrollY());
        return this;
    }

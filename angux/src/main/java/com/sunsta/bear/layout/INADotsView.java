...
public class INADotsView extends View {
    ...
    public static final Property<INADotsView, Float> DOTS_PROGRESS = new Property<INADotsView, Float>(Float.class, "dotsProgress") {
        public Float get(INADotsView object) {
            return object.getCurrentProgress();
        }

        public void set(INADotsView object, Float value) {
            object.setCurrentProgress(value);
        }
    };

    public INADotsView(Context context) {
        super(context);
        this.init();
    }

    public INADotsView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.init();
    }

    public INADotsView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void updateInnerDotsPosition() {
        if (this.currentProgress < 0.3F) {
            this.currentRadius2 = (float) DataService.getInstance().mapValueFromRangeToRange((double) this.currentProgress, 0.0D, 0.30000001192092896D, 0.0D, (double) this.maxInnerDotsRadius);
        } else {
        ...
        if (this.currentProgress == 0.0F) {
            ...
            this.currentDotSize2 = (float) DataService.getInstance().mapValueFromRangeToRange((double) this.currentProgress, 0.20000000298023224D, 0.5D, (double) this.maxDotSize, 0.3D * (double) this.maxDotSize);
        } else {
            this.currentDotSize2 = (float) DataService.getInstance().mapValueFromRangeToRange((double) this.currentProgress, 0.5D, 1.0D, (double) (this.maxDotSize * 0.3F), 0.0D);
        }
    ...
    private void updateOuterDotsPosition() {
        if (this.currentProgress < 0.3F) {
            this.currentRadius1 = (float) DataService.getInstance().mapValueFromRangeToRange((double) this.currentProgress, 0.0D, 0.30000001192092896D, 0.0D, (double) (this.maxOuterDotsRadius * 0.8F));
        } else {
            this.currentRadius1 = (float) DataService.getInstance().mapValueFromRangeToRange((double) this.currentProgress, 0.30000001192092896D, 1.0D, (double) (0.8F * this.maxOuterDotsRadius), (double) this.maxOuterDotsRadius);
        }
        ...
        if (this.currentProgress == 0.0F) {
            ...
            this.currentDotSize1 = (float) DataService.getInstance().mapValueFromRangeToRange((double) this.currentProgress, 0.699999988079071D, 1.0D, (double) this.maxDotSize, 0.0D);
        }
    ...
    private void updateDotsPaints() {
        ...
        if (this.currentProgress < 0.5F) {
            progress = (float) DataService.getInstance().mapValueFromRangeToRange((double) this.currentProgress, 0.0D, 0.5D, 0.0D, 1.0D);
            ...
            progress = (float) DataService.getInstance().mapValueFromRangeToRange((double) this.currentProgress, 0.5D, 1.0D, 0.0D, 1.0D);
    ...
    private void updateDotsAlpha() {
        float progress = (float) DataService.getInstance().clamp((double) this.currentProgress, 0.6000000238418579D, 1.0D);
        int alpha = (int) DataService.getInstance().mapValueFromRangeToRange((double) progress, 0.6000000238418579D, 1.0D, 255.0D, 0.0D);
